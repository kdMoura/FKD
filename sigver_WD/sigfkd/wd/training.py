...
import sigfkd.wd.metrics as metrics
import sigfkd.wd.data as data
...
def test_all_users(classifier_all_user: Dict[int, sklearn.svm.SVC],
    ...
    for user in users:
        ...
        '''#!!!!!!!dts
        num_SF_test = 10 # select 10 (gpds, cedar) or 15 (mcyt) SF for test
        rng = np.random.RandomState(1234)
        skilledPredUser = rng.choice(skilledPredUser, num_SF_test, replace=False) 
        #rng.shuffle(skilledPredUser)
        #skilledPredUser = skilledPredUser[0:num_SF_test]
        #!!!!!!dts'''
...
def test_all_users_dts(classifier_all_user: Dict[int, sklearn.svm.SVC],
                   exp_test: Tuple[np.ndarray, np.ndarray, np.ndarray],
                   num_SF_test: int,
                   global_threshold: float,
                   rng: np.random.RandomState = np.random.RandomState()) -> Dict:
    """ Test classifiers for all users and return the metrics
...
def train_test_all_users_dts(exp_set: Tuple[np.ndarray, np.ndarray, np.ndarray],
                         dev_set: Tuple[np.ndarray, np.ndarray, np.ndarray],
                         svm_type: str,
                         C: float,
                         gamma: float,
